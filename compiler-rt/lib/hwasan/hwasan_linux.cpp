...
static AccessInfo GetAccessInfo(siginfo_t *info, ucontext_t *uc) {
  ...
  bool isFaultShort = ((ebreak & 0x3) != 0x3);
  // faulted insn is not ebreak, not our case?
  if ((ebreak != 0x100073) && ((ebreak & 0xffff) != 0x9002))
    return AccessInfo{};
  pc += isFaultShort ? 2 : 4;
  // reconstruct insn
  byte1 = *((u8*)(pc + 0));
  ...
  // code is encoded in top 12 bits, since instruction is supposed to be with imm
  ...
  if (size_log > 4 && size_log != 0xf)
    return AccessInfo{}; // Not ours.
  const uptr size = size_log == 0xf ? uc->uc_mcontext.__gregs[11] : 1U << size_log;
...
static bool HwasanOnSIGTRAP(int signo, siginfo_t *info, ucontext_t *uc) {
  ...
  bool isFaultShort = ((faulted & 0x3) != 0x3);
